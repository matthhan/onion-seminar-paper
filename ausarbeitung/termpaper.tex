...
\emph{P-Space} construction was, however, so fast that we could not measure runtime very accurately using R's \texttt{system.time} function. However, we point out that even with these inaccurate measurements it is fairly clear that the cost of \emph{P-Space} construction is fairly low and practically linar.
...
%TODO: Add results for ospace OD query
\section{Conclusion}
