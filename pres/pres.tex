...
\author{Matthias Hansen \texttt{(matthias.hansen@rwth-aachen.de)} \\
...
\begin{frame}{Distance-Based Outliers}
    \begin{block}{Distance-Based Outliers}
        Let DB be a set of vectors, dist a distance function.

        $p\in DB$ is a distance-based outlier w.r.t. $k$ and $\varepsilon$
...
\begin{frame}{Performance Considerations}
    \begin{itemize}
        \item Naive: Calculate pairwise distances. Check criterion for each point.
...
\begin{frame}{Parameter Selection is a Problem}
    \begin{itemize}
        ...
        \item ONION (\underline{On}line Outlier Detect\underline{ion}) system:
            ...
            Detect Outliers in logarithmic time.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \end{column}
    \end{columns}
    \visible<3>{Decomposing means solving \textsc{Cliques} $\implies$ \alert{very slow}}
\end{frame}
    \begin{frame}{Contributions}
        \begin{itemize}
            \item Logarithmic-time Outlier Detection 
